module gx4000_crtc6845_plus(
    ...
    output logic [7:0]  hchar_count,
    output logic [6:0]  line_counter,
    output logic [4:0]  raster_counter,
    output logic [7:0]  horizontal_displayed,
    output logic [4:0]  max_raster
);
    logic [7:0] r[0:17];
    logic [4:0] reg_sel;
    logic [7:0] hchar;
    ...
    always_comb begin
        ...
        cpu_rdata = 8'hFF;
        if (data_re && reg_sel < 18)
            cpu_rdata = r[reg_sel];
        hchar_count = hchar;
        line_counter = vrow;
        raster_counter = raster;
        horizontal_displayed = r[1];
        max_raster = r[9][4:0];
    end
    ...
    always_ff @(posedge clk16 or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (select_we)
                reg_sel <= cpu_wdata[4:0];
            if (data_we && reg_sel < 18)
                r[reg_sel] <= cpu_wdata;
